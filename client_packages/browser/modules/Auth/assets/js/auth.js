...
function leaveopacity() {
	...
	main.style.transition="all 0.99s";
}
...
// Добавленный функционал "Запомнить пароль"
document.addEventListener('DOMContentLoaded', function () {
	const savedLogin = localStorage.getItem('savedLogin');
	const savedPassword = localStorage.getItem('savedPassword');

	if (savedLogin && savedPassword) {
		document.getElementById('entry-login-id').value = savedLogin;
		document.getElementById('entry-password-id').value = savedPassword;
		document.getElementById('remember-me-id').checked = true;
	}

	document.querySelector('.js-btn-auth').addEventListener('click', function (event) {
		const login = document.getElementById('entry-login-id').value;
		const password = document.getElementById('entry-password-id').value;
		const rememberMe = document.getElementById('remember-me-id').checked;
...
$(document).ready(() => {
	...
	$(document)
		...
		.on('click', '.char', (e) => {
			...
			U.trigger('chooseChar', slots.currSlotId);
		...
		.on('click', '.choose-chars .choose-the2', () => {
			U.trigger('chooseChar', slots.currSlotId);
			return false;
		})
