...
function enableChatInput(enable) {
    ...
    if (enable != (chat.input != null)) {
        ...
        if (enable) {
            ...
            chat.input = $('<div class="chat-input-wrapper">' +
                '<input value="" id="chat_msg" type="text" autocomplete="off" autofocus />' +
                '<div class="chat-btns">' +
                '<button onclick="doInChat(writeDo)" class="btnchat">/do</button>' +
                '<button onclick="doInChat(writeMe)" class="btnchat">/me</button>' +
                '<button onclick="doInChat(writeTry)" class="btnchat">/try</button>' +
                '<button onclick="doInChat(writeB)" class="btnchat">/b</button>' +
                '</div>' +
                '</div>');

            chatvar.append(chat.input);
            setTimeout(() => {
                chat.input.find("input").focus();
            }, 0);
            chat.input.find("input").focus();
