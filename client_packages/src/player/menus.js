...
mp.events.add('rangesliderboard', (text) => {
	mp.events.callRemote('rangesliderCallback', rangeslider.cBack, text);
    rangeslider.cBack = "";
    rangeslider.close();
});
...
var PetrolMenu = mp.browsers.new('http://package/browser/modules/Petrol/index.html');
// PETROL //
mp.events.add('petrol', (data, diesel, regular, premium) => {
    // mp.events.callRemote('petrol', data, diesel, regular, premium);
	if (regular == true)
	{
	...
	if (diesel == true)
	{
		mp.events.callRemote('petrol_diesel', data);
	}
	if (premium == true)
	{
		mp.events.callRemote('petrol_premium', data);
	}
    //
    global.menuClose();
    PetrolMenu.execute('petrol.reset()');
});
mp.events.add('petrol.full', (diesel, regular, premium) => {
    mp.events.callRemote('petrol', 9999, diesel, regular, premium);
    //
    global.menuClose();
    PetrolMenu.execute('petrol.reset()');
});
mp.events.add('petrol.gov', (diesel, regular, premium) => {
    mp.events.callRemote('petrol', 99999, diesel, regular, premium);
    //
    global.menuClose();
    PetrolMenu.execute('petrol.reset()');
});
...
mp.events.add('open_electropetroltimer', () => {
	...
	{
		// global.menuOpen();
		global.ElectroTimer = mp.browsers.new('http://package/browser/modules/ElectroPetrol/Timer/index.html');
...
var ElectroPetrolMenu = mp.browsers.new('http://package/browser/modules/ElectroPetrol/index.html');

// PETROL //
mp.events.add('electropetrol', (data) => {
    mp.events.callRemote('electropetrol', data);
    //
    global.menuClose();
    ElectroPetrolMenu.execute('petrol.reset()');
});
mp.events.add('electropetrol.full', () => {
    mp.events.callRemote('electropetrol', 9999);
    //
    global.menuClose();
    ElectroPetrolMenu.execute('petrol.reset()');
});
mp.events.add('electropetrol.gov', () => {
    mp.events.callRemote('electropetrol', 99999);
    //
    global.menuClose();
    ElectroPetrolMenu.execute('petrol.reset()');
});
mp.events.add('openElectroPetrol', (price, fuel) => {
    if (global.menuCheck()) return;
    global.menuOpen();
	ElectroPetrolMenu.execute(`petrol.fuel = ${JSON.stringify(fuel)}`);
	ElectroPetrolMenu.execute(`petrol.price = ${JSON.stringify(price)}`);
	ElectroPetrolMenu.execute(`petrol.street = ${mp.game.ui.getStreetNameFromHashKey(mp.game.pathfind.getStreetNameAtCoord(mp.players.local.position.x, mp.players.local.position.y, mp.players.local.position.z, 0, 0).streetName)};`);
	ElectroPetrolMenu.execute('petrol.active=1');
});
mp.events.add('closeElectroPetrol', () => {
    global.menuClose();
    ElectroPetrolMenu.execute('petrol.reset()');
});
// FRACTION MENU //
global.fracMenu = mp.browsers.new('http://package/browser/modules/Pad/index.html');
mp.events.add("openfm", (name, logs, ranks, mats, drug, med, money) => {
  if (global.menuCheck()) return;
  global.menuOpen();
  global.fracMenu.execute("fmenu.active=1");
  global.fracMenu.execute(`fmenu.menu=1`);
  global.fracMenu.execute(`fmenu.btnactive[1]=true`);
  global.fracMenu.execute(`fmenu.submenu=true`);
  global.fracMenu.execute(`fmenu.name='${name}'`);
  global.fracMenu.execute(`fmenu.mats='${mats}'`);
  global.fracMenu.execute(`fmenu.drug='${drug}'`);
  global.fracMenu.execute(`fmenu.med='${med}'`);
  global.fracMenu.execute(`fmenu.moneys='${money}'`);
  global.fracMenu.execute(`fmenu.setlogs(${logs})`);
  global.fracMenu.execute(`fmenu.setranks(${ranks})`);
});

mp.events.add("setmem", (json, count, on, off) => {
  global.fracMenu.execute(`fmenu.set('${json}',${count},${on},${off});`);
});

mp.events.add('fmenu::takemoney', (money) => {
	mp.events.callRemote('fmenutakemoney', money);
	mp.gui.chat.push(`${money}`)
});

mp.events.add('fmenu::putmoney', (money) => {
	mp.events.callRemote('fmenuputmoney', money);
	mp.gui.chat.push(`${money}`)
});
...
mp.keys.bind(Keys.VK_DOWN, false, function () {
...
function createCam(x, y, z, rx, ry, rz, viewangle) {
    ...
    global.cameraRotator.start(camera, vehPosition, vehPosition, new mp.Vector3(-3.0, 3.5, 0.5), 180);
    global.cameraRotator.setZBound(-0.8, 1.8);
    global.cameraRotator.setZUpMultipler(5);
    global.cameraRotator.pause(true);
...
mp.events.add('openAuto', (models, prices) => {
    ...
    global.cameraRotator.pause(false);
...
mp.events.add('openClothes', (price) => {
    ...
    localplayer.position = new mp.Vector3(-724.7034, -381.1296, 34.82832);
    bodyCamStart = localplayer.position;
...
mp.keys.bind(Keys.VK_ALT, false, function () {
    if (!loggedin || chatActive || editing || global.menuOpened && !AltmenuPopUpOpen || new Date().getTime() - lastCheck < 1000) return;
