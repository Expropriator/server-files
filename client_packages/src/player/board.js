global.board = mp.browsers.new('package://browser/modules/Inventory/board.html');
...
mp.events.add('BOARD::LOAD_ASSETS_INFO', (houseData, businessData) => {
...
// 	draggingItem = item;
// 	// Логика для начала перетаскивания
// }
//
// function dragItem(event) {
// 	if (!draggingItem) return;
// 	// Логика для перетаскивания, возможно, обновление положения предмета на экране
// }
//
// function dropItem(event) {
// 	if (!draggingItem) return;
// 	// Логика для обработки того, в какой слот был помещен предмет
// 	let targetSlot = getTargetSlot(event); // Функция, определяющая целевой слот
// 	if (targetSlot !== null) {
// 		mp.events.callRemote('Inventory', 0, targetSlot, 'use');
// 	}
// 	draggingItem = null; // Сброс состояния перетаскивания
// }
//
// function getTargetSlot(event) {
// 	// Определяет, в какой слот был перенесён предмет.
// 	// Может быть базироваться на позиции курсора или вьюпорте
// 	let slots = document.querySelectorAll('.slot');
// 	for (let slot of slots) {
// 		if (isInSlot(event, slot)) {
...
// 	// Проверка, находится ли курсор внутри слота
