...
namespace Golemo
{
    ...
    { 
            ...
            try
            {
                ...
                if (Players.ContainsKey(player))
                {
                    ...
                    try
                    {
                        ...
                        // new jobs
                        Miner.Event_PlayerDisconnected(player, type, reason);
                        Loader.Event_PlayerDisconnected(player, type, reason);
                        Jobs.CustomJob.Construction.Event_PlayerDisconnected(player, type, reason);
                        Jobs.CustomJob.Diver2.Event_PlayerDisconnected(player, type, reason);
                        Jobs.CustomJob.Loader.Event_PlayerDisconnected(player, type, reason);
                        Jobs.CustomJob.Miner.Event_PlayerDisconnected(player, type, reason);
        ...
        [RemoteEvent("Server:HighlightPlayer")]
        public void HighlightPlayer(Player sender, int targetPlayerId)
        {
            var target = NAPI.Pools.GetAllPlayers().FirstOrDefault(p => p.Id == targetPlayerId);
            if (target == null) return;

            sender.TriggerEvent("Client:ShowPlayerHighlight", target.Position.X, target.Position.Y, target.Position.Z);
        }
        ...
        public void ClientEvent_Spawn(Player player, int id)
        {
            ...
            try
            {
                ...
                Notify.Info(player, "Если у Вас возникли какие-то вопросы, обратитесь к Администрации.", 7000);
            ...
            try
            {
                ...
                switch (callback)
                {
                    ...
                    case "sellcar":
                        ...
                        {
                            Notify.Error(player, "Нельзя продавать данное авто");
                            return;
                        }
            ...
            try
            {
                ...
                switch (id)
                {
                    ...
                    case 810: //Fish Market
                        Markets.MarketFish.OpenMarketMenu(player, 0);
                        return;
                    ...
                    #region CustomJobs
                    case 510:
                        Jobs.CustomJob.Diver2.StartWorkDayDiver(player);
                        return;
                    case 508:
                        Jobs.CustomJob.Miner.StartWorkDayMiner(player);
                        return;
                    case 509:
                        Jobs.CustomJob.Construction.StartWorkDayConstruction(player);
                        return;
                    case 5060:
                        Jobs.CustomJob.Loader.StartWorkDayLoader(player);
                        return;
                    #endregion
                    default:
                        return;
                ...
                foreach (Player p in Players.Keys.ToList())
                {
                    try
                    {
                        ...
                        if (!Players[p].IsBonused)
                        {
                            ...
                            var hour = date.Hour;
                            var min = date.Minute;
                            Trigger.ClientEvent(p, "UpdateLastBonus", $"ДО ПОДАРКА ОСТАЛОСЬ {hour}:{min}"); //todo lastbonus
                        }
        ...
        public static void CMD_BUILD(Player client)
        {
            try
            {
                //client.SendChatMessage("!{#6d7bfe}" + $"Все для GTA 5 и RAGE:MP - https://ragemp.pro");
                client.SendChatMessage($"Mirage RolePlay - https://discord.gg/edAJjWN !{{#00FFFF}}");
            }
            catch { }
        }
