...
namespace Golemo.Core.Character
{
    public class Character : CharacterData
    {
            ...
            try
            {
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        ...
                        if (IsBonused)
                        {
                            Trigger.ClientEvent(player, "UpdateLastBonus", $"ВЫ УЖЕ ПОЛУЧИЛИ ПОДАРОК"); //todo lastbonus
                        }
                        else
                        {
                            ...
                            var hour = date.Hour;
                            var min = date.Minute;
                            Trigger.ClientEvent(player, "UpdateLastBonus", $"ДО ПОДАРКА ОСТАЛОСЬ {hour}:{min}"); //todo lastbonus
                        }
                        #endregion
                        ...
                        if (house != null)
                        {
                            ...
                            Trigger.ClientEvent(player, "createGarageBlip", HouseManager.Houses[house.ID].Position);
                        }
        ...
        public async Task Load2(Player player, int uuid)
        {
            try
            {
                if (Main.Players.ContainsKey(player))
        ...
        public async Task<bool> Save(Player player)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (EnterCinema != -1)
                    {
                        Vector3 position = new Vector3(-1423.4061279296875, -215.29371643066406, 46.500423431396484);
                        pos = JsonConvert.SerializeObject(position + new Vector3(0, 0, 1.12));
                    }
                    if (TuningShop != -1)
                    {
                ...
                await MySQL.QueryAsync($"UPDATE `characters` SET `pos`='{pos}',`gender`={Gender},`health`={Health},`armor`={Armor},`lvl`={LVL},`exp`={EXP}," +
                    ...
                    $"`personid`='{PersonID}',`eat`='{Eat}',`water`='{Water}',`demorganreason`='{DemroganReason}',`demorganadmin`='{DemorganAdmin}' WHERE `uuid`={UUID}");
        ...
        public async Task<int> Create(Player player, string firstName, string lastName)
        {
            try
            {
                ...
                SpawnPos = new Vector3(1728.5804, 3320.8928, 41.103504);
                ...
                await MySQL.QueryAsync($"INSERT INTO `characters`(`uuid`,`personid`,`firstname`,`lastname`,`gender`,`health`,`armor`,`lvl`,`exp`,`money`,`bank`,`work`,`fraction`,`fractionlvl`,`arrest`,`fines`,`demorgan`,`wanted`," +
                    $"`workstats`,`biz`,`adminlvl`,`licenses`,`unwarn`,`unmute`,`warns`,`lastveh`,`onduty`,`lasthour`,`lastbonus`,`isbonused`,`luckywheelspins`,`hotel`,`hotelleft`,`contacts`,`achiev`,`sim`,`pos`,`createdate`,`eat`,`water`,`cooldown`,`demorganreason`,`demorganadmin`) " +
                    ...
                    $"'{JsonConvert.SerializeObject(SpawnPos)}','{MySQL.ConvertTime(CreateDate)}','{Eat}','{Water}','{MySQL.ConvertTime(Cooldown)}','{DemroganReason}','{DemorganAdmin}')");
