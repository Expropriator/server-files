    ...
    class Ems : Script
    {
            ...
            try
            {
                ...
                shape.OnEntityEnterColShape += (s, entity) =>
                {
                    try
                    {
                        ...
                        {
                            Notify.Send(entity, NotifyType.Error, NotifyPosition.BottomCenter, "Вы должны закончить лечение чтоб покинуть больницу!", 10000);
        ...
        public void onPlayerDeathHandler(Player player, Player entityKiller, uint weapon)
        {
            try
            {
                ...
                Jobs.CustomJob.Diver2.Event_PlayerDeath(player, entityKiller, weapon);
                Jobs.CustomJob.Construction.Event_PlayerDeath(player, entityKiller, weapon);
                Jobs.CustomJob.Loader.Event_PlayerDeath(player, entityKiller, weapon);
                Jobs.CustomJob.Miner.Event_PlayerDeath(player, entityKiller, weapon);
