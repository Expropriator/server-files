...
namespace Golemo.Jobs
{
    class Taxi : Script
    {
        ...
        // Position of the job marker
        private static Vector3 jobMarkerPosition = new Vector3(895.36, -179.28, 74.70);
        private static Blip jobBlip;
        private static ColShape jobColShape;

        [ServerEvent(Event.ResourceStart)]
        public void OnResourceStart()
        {
            // Create job marker
            // jobColShape = NAPI.ColShape.CreateCylinderColShape(jobMarkerPosition, 1, 2, 0);
            // jobColShape.OnEntityEnterColShape += OnJobMarkerEnter;
            // jobColShape.OnEntityExitColShape += OnJobMarkerExit;

            // Create job blip
            // jobBlip = NAPI.Blip.CreateBlip(jobMarkerPosition);
            // jobBlip.Sprite = 198; // Job blip sprite
            // jobBlip.Color = 5; // Yellow color
            // jobBlip.Name = "Taxi Job";
            // jobBlip.ShortRange = true;

            // Create job marker on the ground
            //NAPI.Marker.CreateMarker(1, jobMarkerPosition - new Vector3(0, 0, 1.0), new Vector3(), new Vector3(), 1.0f, new Color(255, 255, 0, 150), false, 0);
            NAPI.Blip.CreateBlip(198, new Vector3(895.36, -179.28, 74.70), 1.5f, 46, Main.StringToU16("Такси"), 255, 0, true, 0, 0); // Блип на карте
        }

        private void OnJobMarkerEnter(ColShape shape, Player player)
        {
            if (shape == jobColShape)
            {
                player.TriggerEvent("showJobMenu", "Taxi");
            }
        }

        private void OnJobMarkerExit(ColShape shape, Player player)
        {
            if (shape == jobColShape)
            {
                player.TriggerEvent("hideJobMenu");
            }
        }
        ...
        public static void taxiCarsSpawner()
        {
            for (int a = 0; a < CarInfos.Count; a++)
            {
                ...
                // Create a blip for each taxi vehicle
                var blip = NAPI.Blip.CreateBlip(veh.Position);
                blip.Sprite = 56; // Taxi blip sprite
                blip.Color = 5; // Yellow color
                blip.Name = "Taxi";
        ...
        public static void acceptTaxi(Player player, Player target)
        {
            if (Main.Players[player].WorkID == 3 && NAPI.Data.GetEntityData(player, "WORK") != null)
            {
                ...
                {
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Таксист ({player.Value}) принял Ваш вызов. Оставайтесь на месте", 3000);
