...
namespace Golemo.Jobs
{
    class Electrician : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                NAPI.Blip.CreateBlip(354, new Vector3(728.9811, 134.54457, 79.63455), 1.5f, 22, Main.StringToU16("Работа: Электрик"), 255, 0, true, 0, 0); // Блип на карте
                NAPI.TextLabel.CreateTextLabel("~w~Приму вас на работу", new Vector3(728.9811, 134.54457, 79.63455), 30f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension); // Над головой у Ped
                ...
                var col1 = NAPI.ColShape.CreateCylinderColShape(new Vector3(728.9811, 134.54457, 79.63455), 1, 2, 0); // Меню которое открывается на 'E'

                //NAPI.TextLabel.CreateTextLabel("~g~Bobby", new Vector3(728.9811, 134.54457, 79.63455), 30f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                //NAPI.ColShape.CreateCylinderColShape(new Vector3(728.9811, 134.54457, 79.63455), 1, 2, 0);

                // Создание NPC на метке
                //PedHash pedHash = PedHash.Josef;
                //float heading = 0.0f; // Направление, в котором будет смотреть NPC
                //NAPI.Ped.CreatePed(pedHash, new Vector3(728.9811, 134.54457, 79.63455), heading, NAPI.GlobalDimension);

                Vector3 startPosition = new Vector3(724.8585, 134.1029, 81.95643);
                ...
                //Blip jobBlip = NAPI.Blip.CreateBlip(354, startPosition, 1f, 4, "Работа: Электрик", 255, 0, true, 0, 0);
        ...
        [RemoteEvent("playerPressE")]
        public void OnPlayerPressE(Player player)
        {
            try
            {
                if (player.GetData<int>("INTERACTIONCHECK") == 8)
                {
                    if (Main.Players[player].WorkID == 1)
                    {
                        Main.Players[player].WorkID = 0;
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Вы прекратили работу электриком.", 3000);
                    }
                    else
                    {
                        Main.Players[player].WorkID = 1;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы устроились электриком!", 3000);
                    }
                }
            }
            catch (Exception e) { Log.Write("OnPlayerPressE: " + e.Message, nLog.Type.Error); }
        }
