...
namespace Golemo.Jobs
{
    class Trucker : Script
    {
        ...
        public static void OnResourceStart()
        {
            ...
            CreateWorkIDMarker(new Vector3(170.54697, 2777.698, 45.70283));
        }

        public static void CreateWorkIDMarker(Vector3 position)
        {
            var shape = NAPI.ColShape.CreateCylinderColShape(position, 1f, 2f, 0);
            shape.OnEntityEnterColShape += (s, e) =>
            {
                if (e is Player player)
                {
                    Main.Players[player].WorkID = 6;
                    Console.WriteLine($"Player {player.Name} получил WorkID 6.");
                    Notify.Succ(player, "Вы устроились на работу дальнобойщиком!", 3000);
                }
            };
            var marker = NAPI.Marker.CreateMarker(1, position - new Vector3(0, 0, 1.7f), new Vector3(), new Vector3(), 1f, new Color(255, 0, 0, 255), false, 0);
            var textLabel = NAPI.TextLabel.CreateTextLabel("Работа: Дальнобойщик (нажмите E, чтобы устроиться)", position, 3f, 5f, 4, new Color(255, 255, 255), false, 0);
        }
        ...
        public static void OpenTruckerMenu(Player player)
        {
            try
            {
                // Проверяем, может ли игрок открыть меню
                if (!CanWorkingTrucker(player)) return;

                // Проверка на наличие грузовиков в словаре
                if (_trucks == null || _trucks.Count == 0)
                {
                    Log.Write($"OpenTruckerMenu: _trucks dictionary is empty or null for player {player.Name}.", nLog.Type.Warn);
                    Notify.Error(player, "Нет доступных грузовиков для работы.", 3000);
                    return;
                }

                // Получаем уровень игрока
                int playerLevel = Main.Players[player].GetLevelAtThisWork();
                if (playerLevel < 0)
                {
                    Log.Write($"OpenTruckerMenu: Invalid player level ({playerLevel}) for player {player.Name}.", nLog.Type.Warn);
                    Notify.Error(player, "Ваш уровень на данной работе некорректен.", 3000);
                    return;
                }
                ...
                Log.Write($"OpenTruckerMenu successfully opened for player {player.Name}.", nLog.Type.Info);
            }
            catch (Exception ex)
            {
                Log.Write($"Error in OpenTruckerMenu: {ex.Message}", nLog.Type.Error);
                Notify.Error(player, "Произошла ошибка при открытии меню. Попробуйте позже.", 3000);
            }
        }
        #endregion
        ...
        public static void TakeOrder(Player player, int orderType, string truck)
        {
            try
            {
                ...
                if (!_trucks.Values.Contains(truck)) return;
            ...
            {
                Log.Write($"Error in TakeOrder: {e.Message}", nLog.Type.Error);
            }
        }
                ...
                player.SetData("ORDER_LOAD_TIMER", Timers.Start(1000, () =>
                {
                    if (waitSeconds <= 0)
                    {
                        NAPI.Task.Run(() =>
                        {
                            ...
                            {
                                // Обработка случая, когда bizlist пуст
                                Notify.Error(player, "На данный момент нет доступных бизнесов для загрузки.", 3500);
                            }
                        });
        ...
        private static bool CanWorkingTrucker(Player player, string truck = null)
        {
            try
            {
                ...
                Log.Write($"CanWorkingTrucker called for player {player?.Name} with truck: {truck}", nLog.Type.Info);

                // Проверка, существует ли игрок
                if (player == null)
                {
                    Log.Write("Player is null.", nLog.Type.Error);
                    return false;
                }

                // Проверка, есть ли игрок в словаре Main.Players
                if (!Main.Players.ContainsKey(player))
                {
                    Log.Write($"Player {player.Name} not found in Main.Players.", nLog.Type.Error);
                    return false;
                }
                ...
                if (Main.Players[player].WorkID != 6)
                {
                    ...
                    Log.Write($"Player {player.Name} does not have WorkID 6.", nLog.Type.Error);
                    return false;
                }
                ...
                if (player.IsInVehicle)
                {
                    ...
                    Log.Write($"Player {player.Name} is in a vehicle.", nLog.Type.Error);
                    return false;
                }

                // Проверка на активный грузовик игрока
                if (player.HasData("WORK") && player.GetData<Vehicle>("WORK") != null && !string.IsNullOrEmpty(truck))
                {
                    VehicleHash hash = (VehicleHash)NAPI.Util.GetHashKey(truck);
                    Vehicle ptruck = player.GetData<Vehicle>("WORK");

                    if ((VehicleHash)ptruck.Model == hash)
                    {
                        Log.Write($"Player {player.Name} is using the correct truck: {truck}.", nLog.Type.Info);
                        return true;
                    }
                    else
                    {
                        Notify.Error(player, "Прежде чем взять другой грузовик, вам надо сдать прежний", 3500);
                        Log.Write($"Player {player.Name} tried to take another truck without returning the previous one.", nLog.Type.Error);
                        return false;
                    }
                }

                // Проверка на наличие активного заказа
                if (player.HasData("ORDER_BIZ"))
                {
                    Notify.Error(player, "Вам нужно доставить товар, чтобы взять другой заказ", 3500);
                    Log.Write($"Player {player.Name} has an active order and cannot take a new one.", nLog.Type.Error);
                    return false;
                }

                // Если ни одна проверка не вызвала ошибку
                Log.Write($"CanWorkingTrucker: Player {player.Name} is allowed to work as a trucker.", nLog.Type.Info);
                return true;
            }
            catch (Exception ex)
            {
                Log.Write($"Error in CanWorkingTrucker for player {player?.Name}: {ex.Message}", nLog.Type.Error);
                return false;
            }
        }
