...
namespace Golemo.Jobs
{
    class WorkManager : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
             ...
             /*   NAPI.Blip.CreateBlip(354, new Vector3(724.9625, 133.9959, 79.83643), 1.5f, 46, Main.StringToU16("Электростанция"), 255, 0, true, 0, 0);*/
                NAPI.Blip.CreateBlip(351, new Vector3(-260.96762, -904.54315, 31.190844), 0.8f, 3, Main.StringToU16("Почта"), 255, 0, true, 0, 0);
              ...
              //  NAPI.Blip.CreateBlip(269, new Vector3(-1587.9878, 2792.7302, 15.796548), 1f, 31, Main.StringToU16("Fort Zankudo"), 255, 0, true, 0, 0);
        ...
        public static SortedList<int, Vector3> Points = new SortedList<int, Vector3>
        {
            ...
            {8, new Vector3(116.539406, -2575.1301, 6.0045915) },  // AutoMechanic job
            {9, new Vector3(473.9508, -1275.597, 29.60513) },  // Orange job
            {10, new Vector3(473.9508, -1275.597, 29.60513) },  // LumberJack job
        };
        ...
        {
            Trigger.ClientEvent(player, "OpenJobGopostalMenu", Main.Players[player].WorkID);
            Trigger.ClientEvent(player, "NPC.cameraOn", "GOPOSTAL", 1500);
        }
        [RemoteEvent("Server::GoPostalCases")]
        public static void callback_gpStartMenu(Player client, string id)
        {
            switch (id)
            {
                case "start":
                    ...
                    {
                        if (Main.Players[client].WorkID != 0)
                        {
                            Notify.Error(client, "Вы уже работаете на другой работе");
                            return;
                        }
                        if (!NAPI.Data.GetEntityData(client, "ON_WORK"))
                        {
                            ...
                            Jobs.Gopostal.getGoPostalCar(client);
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Succ(client, "Вы уволились с работы почтальона");
                            Main.Players[client].WorkID = 0;
                            Dashboard.sendStats(client);
                        }
                    }
