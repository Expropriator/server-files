...
namespace Golemo.Jobs.CustomJob
{
    class Miner : Script
    {
        ...
        private static int JobsMinLVL = 2; // С какого уровня можно устроиться на работу
        ...
        public void Event_ResourceStart()
        {
            try
            {
                NAPI.Blip.CreateBlip(618, new Vector3(2947.133, 2747.014, 42.28965), 1.5f, 22, Main.StringToU16("Каменоломня"), 255, 0, true, 0, 0); // Блип на карте
        ...
        {
            new Checkpoint(new Vector3(2983.971, 2794.2505, 42.24522), 215.7225), // Взять камень 0
            new Checkpoint(new Vector3(2982.094, 2794.1824, 42.171812), 71.75107), // Взять камень 1
            new Checkpoint(new Vector3(2980.7488, 2797.2405, 42.277888), 357.624), // Взять камень 2
            new Checkpoint(new Vector3(2992.6174, 2776.5134, 41.38906), 17.26049), // Взять камень 3
            new Checkpoint(new Vector3(2989.6436, 2779.685, 41.602824), 111.4968), // Взять камень 4
            new Checkpoint(new Vector3(2998.0815, 2756.2278, 41.330244), 182.0327), // Взять камень 5
            new Checkpoint(new Vector3(2995.9854, 2755.4404, 41.46979), 285.6414), // Взять камень 6
            new Checkpoint(new Vector3(2992.7976, 2803.6128, 42.053645), 154.5374), // Взять камень 7
        };
        private static List<Checkpoint> Checkpoints2 = new List<Checkpoint>()
        {
            new Checkpoint(new Vector3(2962.8162, 2821.87, 42.053255), 178.5201), // Выгрузить камень
        };
        #endregion
        #region Меню 'E'
        ...
        public static void JobJoin(Player player)
        {
            if (Main.Players[player].WorkID != 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы уже работаете: {WorkManager.JobStats[Main.Players[player].WorkID - 1]}", 3000);
                return;
            }
            if (NAPI.Data.GetEntityData(player, "ON_WORK") == true)
            {
            ...
            //Dashboard.sendStats(player);
            //Trigger.ClientEvent(player, "OpenMiner", checkpointPayment, Main.Players[player].LVL, Main.Players[player].WorkID, NAPI.Data.GetEntityData(player, "ON_WORK2"), checkpointPayment2, checkpointPayment3);
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы устроились на работу: {WorkManager.JobStats[JobWorkId - 1]}", 3000);
            var jobsid = Main.Players[player].WorkID;
            Trigger.ClientEvent(player, "secusejob", jobsid);
            return;
        }
