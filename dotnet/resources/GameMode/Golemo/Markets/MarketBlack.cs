...
namespace Golemo.Markets
{
    ...
    class MarketBlack : Script
    {
        ...
        {
            // Координаты для блипа
            Vector3 blackMarketPosition = new Vector3(-1265.422, -1469.5344, 3.0804386);

            // Создаем блип
            Blip blackMarketBlip = NAPI.Blip.CreateBlip(440, blackMarketPosition, 1.0f, 58, "Черный рынок", 255, 0, true, 0, 0);

            // Настраиваем свойства блипа
            blackMarketBlip.ShortRange = true; // Блип виден только на близком расстоянии
            NAPI.Entity.SetEntityDimension(blackMarketBlip, 0); // Устанавливаем измерение (dimension)
