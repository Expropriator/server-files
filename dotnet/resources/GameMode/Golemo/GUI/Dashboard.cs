...
namespace Golemo.GUI
{
    class Dashboard : Script
    {
                    ...
                    case 0:
                        {// self inventory
                            ...
                            {//remove one item from player inventory
                                ...
                                else if (item.Type == ItemType.Dildo || item.Type == ItemType.DildoBl || item.Type == ItemType.DildoRed)
                                {
                                ...
                                else if (item.Type == ItemType.Documents)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Нельзя выбрасывать свои документы", 3000);
                                    return;
                                }
                                else if (item.Type == ItemType.License)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Нельзя выбрасывать свои лицензии", 3000);
                                    return;
                                }
                                ...
                                if (item.Count > 1)
                                {
                                    ...
                                    Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_drop");
                                    return;
                                }
                                    ...
                                    case 2:
                                        {
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_transfer_toveh");
                                                return;
                                            }
                                    ...
                                    case 3:
                                        {
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_transfer_tosafe");
                                                return;
                                            }
                                    ...
                                    case 5:
                                        {
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_transfer_toplayer");
                                                return;
                                            }
                                    ...
                                    case 6:
                                        {
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_transfer_tofracstock");
                                                return;
                                            }
                    ...
                    case 2:
                        { // in car items
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_transfer_fromveh");
                                return;
                            }
                    ...
                    case 3: // Взять
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_transfer_fromsafe");
                                return;
                            }
                    ...
                    case 6:
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openModalRangeSlider", item.Count, "item_transfer_fromfracstock");
                                return;
                            }
        ...
        // Пример функции для получения названия по ID
        private string GetWorkNameById(int id)
        {
            var names = new Dictionary<int, string>()
    {
        ...
        {7, "инкассатор" },
        {8, "автомеханик" },
        {9, "Сборщик апельсинов" },
        {10, "Лесоруб" },
        // Добавь остальные работы
    };

            return names.ContainsKey(id) ? names[id] : "Неизвестно";
        }

        #region Remote Events
        [RemoteEvent("Server:RequestAllWorkLevels")]
        public static void RequestAllWorkLevels(Player player)
        {
            NAPI.Util.ConsoleOutput($"[DEBUG] RequestAllWorkLevels вызван от игрока {player.Name}");
            if (!Main.Players.ContainsKey(player)) return;
            Character data = Main.Players[player];

            var character = Main.Players[player];

            var works = character.WorksStats.Select(ws => new {
                ws.WorkID,
                ws.Level,
                ws.Exp,
                ws._maxLevel,
                ws._expCountForUpLevel
            }).ToList();

            NAPI.Util.ConsoleOutput($"[DEBUG] Кол-во работ у игрока {player.Name}: {character.WorksStats.Count}");

            player.TriggerEvent("Client:ReceiveAllWorkLevels", JsonConvert.SerializeObject(works));
            NAPI.Util.ConsoleOutput($"[DEBUG] Отправлены уровни работ клиенту: {JsonConvert.SerializeObject(works)}");
        }
        ...
        public static void sendStats(Player player)
        {
            try
            {
                ...
                string status =
                    ...
                    (Main.Accounts[player].VipLvl > 0) ? $"VIP {Group.GroupNames[Main.Accounts[player].VipLvl]} до {Main.Accounts[player].VipDate.ToString("dd.MM.yyyy")}" :
        ...
        public static void sendItems(Player player)
        {
            try
            {
                ...
                string json = JsonConvert.SerializeObject(data);
                Log.Debug(json);
                Trigger.ClientEvent(player, "board", 3, json, nInventory.GetWeight(nInventory.Items[Main.Players[player].UUID]));
        ...
        public static async Task SendItemsAsync(Player player)
        {
            try
            {
                ...
                NAPI.Task.Run(() => Trigger.ClientEvent(player, "board", 3, json, nInventory.GetWeight(nInventory.Items[Main.Players[player].UUID])));
        ...
        public static void Update(Player Player, nItem item, int index)
        {
            ...
            Trigger.ClientEvent(Player, "board", 6, json, index, nInventory.GetWeight(nInventory.Items[Main.Players[Player].UUID]));
        }
        public static Task UpdateAsync(Player Player, nItem item, int index)
        {
            try
            {
                ...
                string json = JsonConvert.SerializeObject(idata);
                NAPI.Task.Run(() => Trigger.ClientEvent(Player, "board", 6, json, index, nInventory.GetWeight(nInventory.Items[Main.Players[Player].UUID])));
            }
